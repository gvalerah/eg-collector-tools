...
def Collector_2(C,ini_file):
    C.logger.info("%s: Demo ENJOY Collector: Execution start"%(__name__))
    C.logger.info("%s: Reading configuration file: %s"%(__name__,ini_file))
    ...
    if (os.path.isfile(ini_file)):
        ...
        if active == 'True':
            ...
            nutanix_sql_file = strftime("service/sql/%Y%m%d_demo_enjoy.sql")    
