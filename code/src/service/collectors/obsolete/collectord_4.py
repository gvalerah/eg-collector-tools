...
def Collector_4(C,ini_file):
    C.logger.info("%s: Collector data Injector Demo Falabella: Execution start"%(__name__))
    C.logger.info("%s: ini file = %s"%(__name__,ini_file))
    ...
    if (os.path.isfile(ini_file)):    
        ...
        if active == 'True':
            ...
            nutanix_sql_file = strftime("service/sql/%Y%m%d_demo_falabella.sql")
