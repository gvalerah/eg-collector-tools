...
def Nutanix_VGroup_Load_Collector(C,config,group):

    C.logger.info(f"{this()}: Nutanix Volume Groups Storage Loader : Execution start")
    if True:    
        ...
        for node in nodes:
            if active:
                ...
                C.logger.info(f"{this()}: Nutanix Node to search for volume groups = {node} : {N.username}@{N.host}")  
                ...
                while N.has_more_data:
                    retcode=N.Extract_VGroups(API_version=2)
                    if retcode is not None and retcode==200:
                        ...
                        N.has_more_data=False                    
                C.logger.debug(f"{this()}: out loop has more data = {N.has_more_data} total={N.total_matches} processed={N.processed_vgroups}")
                good_bye_message=f" {N.processed_vgroups} entities processed out of {N.total_matches} matched in platform '{name}'."
            else:
                C.logger.info(f"{this()}: Collector Inactive.")        
    else:
            C.logger.critical(f"{this()}: Configuration file '{nutanix_ini_file}' does not exist.")
    C.logger.info(f"{this()}: Nutanix Volume Groups Storage Loader : Completed")
