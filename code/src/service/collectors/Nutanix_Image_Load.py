...
def Nutanix_Image_Load_Collector(C,config,group):
    ...
    C.logger.info(f"{__name__}: Nutanix Image List Storage Loader : Execution start")
    if True:    
        ...
        for node in nodes:
            if active:
                ...
                C.logger.debug(f"{__name__}: out loop has more data = {N.has_more_data} total={N.total_matches} processed={N.processed_images}")
    ...
    C.logger.info("{__name__}: Nutanix Image List Storage Loader : Completed")
