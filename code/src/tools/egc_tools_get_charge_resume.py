...
def Get_Charging_Resume(args=None):
    ...
    if args.update:
        ...
        if args.customer_id is not None and args.cost_center is None:
        ...
        if args.fast:
            ...
            records = db.Update_Charge_Resume_CIS(
                ...
                callback_step=args.feedback_step
                )
    ...
    if type(rows) == list:
        if len(rows):
            logger.info(f"{this()}: PRE RENDER")
            logger.info(f"{this()}: FILTER CUSTOMER = {FILTER_CUSTOMER}")
            logger.info(f"{this()}: type rows        = {type(rows)}")
            try:
                logger.info(f"{this()}: len rows         = {len(rows)}")
            except:
                logger.warning(f"{this()}: len rows         = ERROR")        
            logger.info(f"{this()}: Cus_Id           = {args.customer_id}")
            #print(f"{this()}: Cus_Name         = {Cus_Name}")
            logger.info(f"{this()}: CIT_Date_From    = {args.start}")
            logger.info(f"{this()}: CIT_Date_To      = {args.end}")
            logger.info(f"{this()}: CIT_Status       = {args.cit_status}")
            logger.info(f"{this()}: Cur_Code         = {args.currency}")
            logger.info(f"{this()}: template         : report_charging_resume.html")
            try:
                """
