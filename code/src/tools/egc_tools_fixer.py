...
def cis_fix(session,args):
    ...
    print("period         =",period)
    start,end = Get_Period(period)
    print("Charge Items:",Charge_Items.__tablename__,Charge_Items.__table__.name)
    Charge_Items.__tablename__  = 'Charge_Items_%s'%Period
    Charge_Items.__table__.name = 'Charge_Items_%s'%Period
    print("Charge Items:",Charge_Items.__tablename__,Charge_Items.__table__.name)
    print("period    start,end      =",start,end)
    now_start,now_end = Get_Period(datetime.now(),PERIOD_HOUR)
    print("now       start,end      =",now_start,now_end)
    if end > now_end:
        end = now_end
    print("effective start,end      =",start,end)
    tsstart=int(datetime.timestamp(start))
    tsend=int(datetime.timestamp(end))
    slices=[]
    for slice in range(tsstart,tsend,3600):
    ...
    if args.filename is None:
        Filename=input("Status filename ? ")
    else:
        Filename=args.filename    
    
    # Leer archivo de status a pandas data frame
    # pd.read_excel(filename,sheet[0])
    df=pd.read_csv(Filename,sep=';')
    #print(df)
    # for row in df.itertuples(index=True, name='Pandas'):
    
    # por cada linea en dataframe
    cits_generated=cits_errors=cits_present=cits_expected=0
    #for row in df:
    x=0
    #for row in df.itertuples(index=True, name='Pandas'):
    print()
    ...
    total_rows=df['CI'].count()
    tups=namedtuple("row","CI CU NAME UUID verrecs verrecsp mean var stddev min max n missing missingp XXX")
    for row in df.itertuples(index=False, name='Pandas'):
        ...
        advance_now=x/total_rows
        expected_ts=now_ts+(elapsed_seconds*(1-advance_now)/advance_now)
        # si stddev == 0
        # print(row)
        # print(row.CU,row.stddev,row.mean)
        #if x>2:
        #    break
        #print("df['CI'].count()",df['CI'].count())
        print("\r%6.02f%% %s/%s ETA=%s "%(
            advance_now*100,
            x,total_rows,
            datetime.fromtimestamp(expected_ts)),end=''
            )
        if row.stddev == 0:
            # por cada slice
            try:
                cits_expected+=len(slices)
                for slice in slices:
                    # Ojo hay que filtrar CI Commissioning Date aqui
                    
                    query=session.query(Charge_Items
                        ).join(Charge_Units,Charge_Units.CU_Id==Charge_Items.CU_Id
                        ).join(Configuration_Items,Configuration_Items.CI_Id==Charge_Units.CU_Id
                        ).filter(Charge_Items.CU_Id==row.CU
                        ).filter(Charge_Items.CIT_DateTime==slice
                        ).filter(Configuration_Items.CI_Commissioning_DateTime<=slice
                        )
                    """
                    query=session.query(Charge_Items
                        ).filter(Charge_Items.CU_Id==row.CU
                    ...
                    """
                    #print("query=",query)
                    CIT=query.one_or_none()
                    # si slice es ausente                
                    if CIT is None:
                        print("+",end='',flush=True)
                        #print(row.CU,slice,"ausente")
                        # genera cit 
                        CIT=Charge_Items(
                            CU_Id=row.CU,
                            CIT_Quantity=row.mean,
                            CIT_Status=1,
                            CIT_Is_Active=1,
                            CIT_Date=None,
                            CIT_Time=None,
                ...
                cits_errors+=1
                print()
                print("Exception: CU=%s %s"%(row.CU,str(e)))
                print("CITS expected=%s present=%s %.1f%% generated=%s %.1f%% errors=%s %.1f%%"%(
                    cits_expected,
                    cits_present,cits_present*100/cits_expected,
                    cits_generated,cits_generated*100/cits_expected,
                    cits_errors,cits_errors*100/cits_expected,
                    ))
                print()
    print()
    print()
    ...
    while True:
        ...
        if len(option):
            ...
            if f != -1:
                ...
                elif opc=='cis-fix'          : cis_fix(session,args)
                print()
...
valid_modes =   {   #'create-cit-shards':'Create CIT shard tables',
                    ...
                    'cis-fix':'CIs integrity fix in period',
