...
class Context:
    ...
    def Set_Engine(self):    
        ...
        self.engine_string=str("%s+%s://%s:%s@%s:%s/%s"%(self.rdbms,self.dialect,self.user,self.password,self.host,self.port,self.schema))
        try:
            # 20190820 GV self.ormdb=Collector_ORM_DB(rdbms=self.rdbms,dialect=self.dialect,host=self.host,port=self.port,user=self.user,password=self.password,instance=self.schema)
            # 20190820 GV self.ormdb.Connect()
            # 20190819 GV self.db     = create_engine(self.engine_string)
            self.db     = create_engine(self.engine_string)
            #self.engine = self.db
            self.logger.debug("%s: Create Engine success with [%s]"%(__name__,self.engine_string))
        except Exception as e:
            self.logger.error("%s: Create Engine fails [%s]"%(__name__,self.engine_string))
            self.logger.error("%s: Exception [%s]"%(__name__,e))
    """
"""
