...
class Charge_Resumes(Base):
    ...
    def search_key(self,Cus_Id,CR_Date_From,CR_Date_To,CIT_Status,Cur_Code,CU_Id):
        """ Search for an unique Charge_Resumes record using all key fields (Cus_Id,CR_Date_From,CR_Date_To,CIT_Status,Cur_Code,CU_Id) """
        try:
            if self.engine is not None:
                Session=sessionmaker(bind=self.engine)
                session=Session()
                record = session.query(Charge_Resumes).filter(Charge_Resumes.Cus_Id==Cus_Id).filter(Charge_Resumes.CR_Date_From==CR_Date_From).filter(Charge_Resumes.CR_Date_To==CR_Date_To).filter(Charge_Resumes.CIT_Status==CIT_Status).filter(Charge_Resumes.Cur_Code==Cur_Code).filter(Charge_Resumes.CU_Id==CU_Id).one_or_none()
                session.close()
            else:
                session.rollback()
                record = None
        except Exception as e:
            detail='Charge_Resumes.search_key(%s,%s,%s,%s,%s,%s): Exception: %s'%(Cus_Id,CR_Date_From,CR_Date_To,CIT_Status,Cur_Code,CU_Id,e)
...
class Customers(Base):
    ...
    def get_column_types(self):
        ...
        __list=['Integer', 'String(45)', 'Integer']

        return __list

    def get_column_meta(self):
        """ Gets Customers record column data meta list """
        __list=[('Cus_Id', 'Integer'), ('Cus_Name', 'String(45)'), ('CC_Id', 'Integer')]
...
class Platforms(Base):
    ...
    def get_column_types(self):
        ...
        __list=['Integer', 'String(45)', 'String(45)', 'String(45)', 'String(45)', 'String(45)']

        return __list

    def get_column_meta(self):
        """ Gets Platforms record column data meta list """
        __list=[('Pla_Id', 'Integer'), ('Pla_Name', 'String(45)'), ('Pla_Host', 'String(45)'), ('Pla_Port', 'String(45)'), ('Pla_User', 'String(45)'), ('Pla_Password', 'String(45)')]
